...
async def analyze_email(req: EmailAnalyzeRequest):
    ...
    for pattern in config.ignore_senders:
        ...
        if re.search(regex_pattern, req.sender, re.IGNORECASE):
            logger.info(f"🚫 Email ignored: sender {req.sender} matches blacklist pattern {pattern}")
...
"""
    ...
    try:
        ...
        if importanza not in allowed_priorities:
            logger.warning(f"⚠️ LLM hallucinated priority '{importanza}'. Falling back to LOW.")
            importanza = "LOW"
        ...
        if email_prio_val < min_prio_val:
            logger.info(f"🚫 Email ignored: Priority '{importanza}' is below threshold '{config.min_priority}'")
