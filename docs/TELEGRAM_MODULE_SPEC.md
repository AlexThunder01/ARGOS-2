...
- Expose a public Telegram bot that responds in natural language via LLM (Groq)
...
[FastAPI /telegram/chat]
   ...
   │
   ├─► [Groq Embeddings API] embed(text)
   │      → float vector[768]
   │
   ...
   │
   ├─► [Groq LLM API] chat completion (llama-3.3-70b-versatile)
...
> **Change**: Removed `embedding BLOB` from conversations table. Embeddings are only stored in the dedicated `tg_memory_vectors` table. Conversation rows are for sliding window context only — embedding every single message would double Groq API calls for zero retrieval benefit.
...
### 5.2 Layer 2 — RAG with Groq Embeddings

Long-term memories use embeddings from Groq API and cosine similarity computed in Python with `numpy`.
...
>
> This reduces Groq API calls by ~70% compared to extracting on every turn. Short messages like "ok", "thanks", "yes" are never analyzed.
...
- [ ] Verify Groq Embeddings API availability (`nomic-embed-text-v1.5`)
...
### Why Groq Embeddings and not local models
A local embedding model (`sentence-transformers`) would require ~500MB additional RAM. Groq offers `nomic-embed-text-v1.5` with <100ms latency at negligible cost.
