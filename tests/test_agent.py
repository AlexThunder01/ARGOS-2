...
def test_call_openai_compatible(mock_post):
    ...
    mock_response.json.return_value = {"choices": [{"message": {"content": "Hello OpenAI!"}}]}
...
def test_openai_compatible_key_rotation(mock_post, mock_sleep):
    ...
    call_2.json.return_value = {"choices": [{"message": {"content": "Hello on retry!"}}]}
    
    mock_post.side_effect = [call_1, call_2]

    agent = JarvisAgent()
    # Mock config to have 2 keys
    with patch('src.agent.LLM_API_KEY_2', 'second_key', create=True), \
    ...
    import src.agent
    with patch.multiple(sys.modules['src.agent'], LLM_API_KEY='f', LLM_API_KEY_2='s', create=True):
